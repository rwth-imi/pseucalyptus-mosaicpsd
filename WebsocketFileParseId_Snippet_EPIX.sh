...
function worker() {
	...
	curl --silent -X POST -H "x-client-id: $clientId" -H "x-client-domain: $clientDomain" -H "x-accessable-by: GPAS" -H 'Content-Type: text/csv' --data-binary @<(curl --silent -X GET -H "x-client-id: $clientId" -H "x-client-domain: $clientDomain" "http://localhost:3000/v1/transactions/$transactionId/processes/$processId/files/$fileId" | ../../mosaic-cli-chunked/epix-cli-chunked.sh) "http://localhost:3000/v1/transactions/$transactionId/processes/$pid/files/$fileId"
}
